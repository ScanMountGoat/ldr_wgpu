...
pub fn create_scan_pipeline(device: &wgpu::Device) -> wgpu::ComputePipeline {
    let shader = shader::scan::create_shader_module(device);
    let render_pipeline_layout = shader::scan::create_pipeline_layout(device);
